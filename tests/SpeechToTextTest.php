...
final class SpeechToTextTest extends TestCase
{
    ...
    public function setUp(): void
   {
	...
	public function testSpeechToTextAllParameters()
	{
		$params = [
			## ================= *Required Parameters  =================
			'linkFile' => 'Put voice file link',
		...
		try {
			$result = $AiService->speechToText($params);
	...
	public function testSpeechToTextRequiredParameters()
	{
		$params = [
			## ================= *Required Parameters  =================
			'linkFile' => 'Put voice file link',
	...
	public function testSpeechToTextValidationError()
	{
		...
		try {
			self::$AiService->speechToText($paramsWithoutRequired);
		} catch (ValidationException $e) {
			...
			$this->assertArrayHasKey('linkFile', $validation);
			$this->assertEquals('The property linkFile is required', $validation['linkFile'][0]);
		...
		try {
			self::$AiService->speechToText($paramsWrongValue);
		} catch (ValidationException $e) {
			...
			$result = $e->getResult();
			$this->assertArrayHasKey('linkFile', $validation);
			$this->assertEquals('Integer value found, but a string is required', $validation['linkFile'][1]);
